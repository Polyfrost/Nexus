...
import { cachedFetch } from '~/utils.server';
...
const printError = (e: Error) => console.warn(`[error] /src/util/getContributors.ts\n ${e?.message ?? e}`);
...
async function recursiveFetch(endpoint: string, page = 1) {
	try {
		...
		const res = await cachedFetch(
			url,
			{
				...
				headers: {
					...
					'User-Agent': 'polyfrost-docs-client/1.0',
				},
			},
			{ duration: '15m' },
		);
		...
		if (!res.ok)
			throw new Error(`Request to fetch endpoint failed. Reason: ${res.statusText} Message: ${data?.message}`);
...
export async function getTranslationContributors(paths: string[], repo: string) {
	...
	for (const path of paths) {
		...
		for (const { author, commit } of commits) {
			if (ignoredCommitKeywords.test(commit.message) || !author)
				continue;
